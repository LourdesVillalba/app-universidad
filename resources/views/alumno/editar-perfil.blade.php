...
<div class="h-full flex items-center justify-center px-4 py-8">
    <div class="w-full max-w-4xl mx-auto overflow-hidden bg-white rounded-lg shadow-md p-6">
        ...
        <form action="{{ route('perfil.update') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {{-- Columna Izquierda --}}
            <div class="space-y-4">
                @foreach (['nombre', 'apellido', 'dni', 'carrera', 'comision'] as $campo)
                    <input type="text" name="{{ $campo }}" placeholder="{{ ucfirst($campo) }}" 
                        value="{{ old($campo, $perfil->$campo) }}"
                ...
                <div class="flex flex-col items-center justify-center w-full">
                    ...
                    @if($perfil->foto)
                        <div class="mt-4 text-center">
                            ...
                            <img src="{{ asset('storage/' . $perfil->foto) }}" alt="Foto actual" class="w-24 h-24 rounded mx-auto">
